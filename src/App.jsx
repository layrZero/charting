...
export default function App() {
  ...
  return <main className="terminal-shell">
    ...
    <section className={`terminal-grid panel-${mobilePanel}`}><article className="chart-panel"><ChartTerminal client={client} active={workspace.active} onActiveChange={updateActive} theme={workspace.theme} forecastMode={workspace.forecast?.mode || 'rolling-10'} forecastDiagnosticsVisible={workspace.forecast?.diagnosticsVisible !== false} forecastUncertaintyVisible={workspace.forecast?.uncertaintyVisible !== false} onForecastUncertaintyVisible={(visible) => setWorkspace((w) => ({ ...w, forecast: { ...w.forecast, uncertaintyVisible: visible } }))} /></article>{workspace.panels.optionChain && <aside className="options-panel"><OptionChainPanel client={client} active={workspace.active} onSelect={(contract) => updateActive({ symbol: contract.symbol, exchange: contract.exchange || 'NFO' })} /></aside>}<aside className="trading-panel"><TradingPanel client={client} active={workspace.active} strategy={workspace.trading?.strategy || ''} onStrategyChange={(strategy) => setWorkspace((previous) => ({ ...previous, trading: { ...previous.trading, strategy } }))} /></aside><aside className="portfolio-panel"><PortfolioPanel client={client} /></aside></section>
