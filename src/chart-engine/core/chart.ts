...
export class Chart {
   ...
   * `@layr0/chart-engine/indicators` tier — import it (or register your own with
   ...
   *
   * ```ts
   * import '@layr0/chart-engine/indicators';
  ...
  private _newBackend(): IRenderBackend {
    ...
    if (backend === null) throw new Error('@layr0/chart-engine: the canvas2d render backend factory returned null');
    if (this._rendererKind === null) {
      ...
      if (this._requestedRenderer !== null && this._requestedRenderer !== 'auto' && backend.kind !== this._requestedRenderer) {
        ...
        console.warn(`@layr0/chart-engine: render backend "${this._requestedRenderer}" is unavailable on this device; using "${backend.kind}"`);
      }
    }
    return backend;
  }
