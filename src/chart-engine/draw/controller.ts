...
// so this survives as `from '@layr0/chart-engine'` and stays one identity.
import type { IPrimitive, DataLayer } from '@layr0/chart-engine';
import type {
...
export class DrawingController {
  ...
  public setTool(toolId: string | null): void {
    if (toolId !== null && !hasDrawingTool(toolId)) {
      throw new Error(`@layr0/chart-engine: unknown drawing tool "${toolId}"`);
    }
    this._tool = toolId;
    this._pending = [];
