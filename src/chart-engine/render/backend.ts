...
export function resolveRenderBackend(choice: RendererChoice = 'canvas2d'): RenderBackendFactory {
  ...
  if (fallback === undefined) throw new Error('@layr0/chart-engine: no canvas2d render backend is registered');
  if (choice === 'auto') {
  ...
  if (factory === undefined) {
    throw new Error(`@layr0/chart-engine: render backend "${choice}" is not registered; import the tier that provides it first`);
  }
