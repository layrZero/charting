...
function compile(gl: WebGL2RenderingContext, type: number, source: string): WebGLShader {
  ...
  if (shader === null) throw new Error('@layr0/chart-engine: could not create a WebGL shader');
  gl.shaderSource(shader, source);
  gl.compileShader(shader);
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS) && !gl.isContextLost()) {
    const log = gl.getShaderInfoLog(shader) ?? '';
    gl.deleteShader(shader);
    throw new Error(`@layr0/chart-engine: WebGL shader failed to compile: ${log}`);
  }
  return shader;
}
...
export function compileShapeProgram(gl: WebGL2RenderingContext): ShapeProgram {
  ...
  if (program === null) throw new Error('@layr0/chart-engine: could not create a WebGL program');
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS) && !gl.isContextLost()) {
    ...
    throw new Error(`@layr0/chart-engine: WebGL program failed to link: ${log}`);
  }
  return {
    program,
