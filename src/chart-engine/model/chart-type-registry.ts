...
export function getChartType(type: SeriesType | (string & {})): RendererEntry {
  ...
  if (e === undefined) {
    if (TRANSFORM_TIER_TYPES.has(type)) {
      throw new Error(`@layr0/chart-engine: series type "${type}" needs the transform tier — import '@layr0/chart-engine/transform' first`);
    }
    throw new Error(`@layr0/chart-engine: unknown series type "${type}"`);
  }
  return e;
}
