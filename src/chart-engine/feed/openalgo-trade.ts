...
export class OpenAlgoTradeFeed implements OrderFeed {
  ...
  public constructor(config: OpenAlgoTradeConfig) {
    this._config = config;
    this._strategy = config.strategy ?? '@layr0/chart-engine';
  ...
  private async _post(path: string, body: Record<string, unknown>): Promise<unknown> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`@layr0/chart-engine: ${path} failed (${res.status})${detail}`);
    }
  ...
  public async getServerMode(maxAgeMs = this._modeCacheMs): Promise<TradeMode> {
    ...
    if (mode === undefined) {
      throw new Error('@layr0/chart-engine: /api/v1/analyzer/ returned no readable mode');
    }
  ...
  private async _assertMode(expected: TradeMode): Promise<void> {
    ...
    if (actual === undefined && (this._verifyMode === 'always' || expected === 'analyzer')) {
      ...
      try {
        ...
        throw preflight(`@layr0/chart-engine: cannot confirm the server mode, no order was sent (${String((err as Error).message ?? err)})`);
      }
    }
    if (actual === undefined || actual === expected) return;
    throw preflight(
      `@layr0/chart-engine: refusing to place, caller expects ${expected} mode but the OpenAlgo server is in ${actual} mode`,
    );
  }
  ...
  private _assertQuantity(req: PlaceRequest): void {
    ...
    if (!v.ok) {
      ...
      throw preflight(`@layr0/chart-engine: ${req.symbol} ${exchange}: ${v.reason}`);
    }
  }
  ...
  private _claimToken(token: string | undefined): string | undefined {
    ...
    if (held !== undefined) {
      throw preflight(
        held === 'ambiguous'
          ? `@layr0/chart-engine: clientToken ${token} may already be live at the broker; check the order book before retrying`
          : `@layr0/chart-engine: duplicate clientToken ${token}`,
      );
    }
  ...
  public async place(req: PlaceRequest & { mode: TradeMode }): Promise<{ orderId: string }> {
    ...
    if (orderId === undefined) throw new Error('@layr0/chart-engine: placeorder returned no orderid');
  ...
  public async modify(orderId: string, patch: { price?: number; triggerPrice?: number; qty?: number }): Promise<void> {
    ...
    if (ctx === undefined) {
      ...
      throw preflight(`@layr0/chart-engine: cannot modify ${orderId}, unknown order context (place it or load the order book first)`);
    }
