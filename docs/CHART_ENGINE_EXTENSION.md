...
The overlay consumes only completed IMC history. It never uses LTP/depth messages to manufacture candles. Native TimesFM P10/P25/P50/P75/P90 close quantiles are rendered as separate lines. A local SQLite-backed walk-forward calibration can replace those lines with calibrated diagnostics, while P50 multivariate output supplies the forecast candle values.
