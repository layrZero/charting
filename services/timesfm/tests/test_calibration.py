...
class CalibrationStoreTests(unittest.TestCase):
    ...
    def test_stable_scope_reuses_ready_record_for_small_history_refresh(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            store.begin(run_id='run', context_key='exact-old', stable_context_key=scope, symbol='A', exchange='NSE', interval='1m', history_fingerprint='exact-old', model_id='model', model_revision='rev', version=1, origins=32, history_bars=600)
            store.complete('run', {'1': {'P10': 1.0}}, {'1': {'P10': 32}}, [(1, 1, {'P10': 10}, 11)])
            reused = store.get_reusable(stable_context_key=scope, symbol='A', exchange='NSE', interval='1m', model_id='model', model_revision='rev', version=1, ttl_seconds=86400, minimum_new_bars=16, current_bar_count=615)
            self.assertEqual(reused['run_id'], 'run')
            self.assertIsNone(store.get_reusable(stable_context_key=scope, symbol='A', exchange='NSE', interval='1m', model_id='model', model_revision='rev', version=1, ttl_seconds=86400, minimum_new_bars=16, current_bar_count=616))
