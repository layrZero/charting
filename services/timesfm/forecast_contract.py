"""Validation and JSON-safe helpers for the TimesFM forecast service."""
...
def normalize_predictions(rows, timestamps):
    ...
    for timestamp, row in zip(timestamps, rows, strict=True):
        candle = {'time': int(timestamp), 'open': float(row['open']), 'high': float(row['high']), 'low': float(row['low']), 'close': float(row['close']), 'volume': max(0.0, float(row.get('volume', 0) or 0))}
        ...
        if not all(isfinite(value) for value in candle.values()):
            raise ValueError('TimesFM returned non-finite values')
        candles.append(candle)
    if len(candles) != 10:
        raise ValueError('TimesFM must return exactly ten candles')
    return candles
